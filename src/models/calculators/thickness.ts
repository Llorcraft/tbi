...
export class ThicknessSurfaceAndPipe extends BaseCalculator {
...
export class ThicknessFlageAndValve extends BaseCalculator {
    constructor(protected report: ReportBase, protected fnc: Function[]) { 
        super(report, fnc);
    }
    protected get_l(index: number) {
        if (this.Δθ <= 80) return [20, 100][index] / 1000;
        if (this.Δθ <= 150) return [30, 150][index] / 1000;
        if (this.Δθ <= 250) return [50, 200][index] / 1000;
        return [80, 250][index] / 1000;
    }
}